...
var activeDocument = typeof window !== "undefined" && window.document ? window.document : null;
...
var NoteDrawPlugin = class extends import_obsidian.Plugin {
  ...
  pickHeaderController(view, state, preferred = null) {
    const controllers = Array.from(state.controllers || []).filter((controller) => controller?.previewEl?.isConnected && controller.view === view);
  ...
  scheduleDrawingSave(file, data) {
    ...
    const timer = window.setTimeout(() => {
      ...
      this.writeDrawings(file, data).catch((error) => {
        ...
        new import_obsidian.Notice("Failed to save drawing data.");
      });
    }, 500);
...
var PreviewDrawingController = class {
  ...
  onPointerDown(event) {
    ...
    const editable = this.allowTextEdit && this.toolMode !== TOOL_TEXT ? findEditableTarget(target, this.previewEl) : null;
    ...
    if (hitStrokeIndex >= 0 && isTextStroke(this.drawingData.strokes[hitStrokeIndex]) && (this.toolMode === TOOL_TEXT || this.toolMode === TOOL_SELECT)) {
  ...
  onPointerMove(event) {
    ...
    if (!this.currentStroke && (this.toolMode === TOOL_TEXT || this.pointerStartTextStrokeIndex >= 0)) {
      ...
      if (this.toolMode === TOOL_TEXT && this.didMove && event.pointerType === "touch" && this.pointerStartTextStrokeIndex < 0) {
  ...
  openFloatingTextInput(point, index = -1) {
    ...
    const state = {
      ...
      anchorClientPoint,
      index,
      originalPoints: isTextStroke(existing) ? existing.points.map((strokePoint) => ({ ...strokePoint })) : null,
  ...
  isFloatingTextInputEditing(index) {
    return this.floatingTextInput?.index === index && index >= 0 && isTextStroke(this.drawingData.strokes[index]);
  }
...
var NoteDrawSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  renderSettings() {
    const { containerEl } = this;
    const settings = sanitizeSettings(this.plugin.settings);
    new import_obsidian.Setting(containerEl).setName("Default pen color").setDesc("Initial color for new pen strokes.").addColorPicker((component) => component.setValue(settings.defaultPenColor).onChange(async (value) => {
      this.plugin.settings.defaultPenColor = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default pen width").setDesc("Initial pen width.").addSlider((component) => component.setLimits(MIN_BRUSH_WIDTH, MAX_BRUSH_WIDTH, 0.5).setValue(settings.defaultPenWidth).onChange(async (value) => {
      this.plugin.settings.defaultPenWidth = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default pen opacity").setDesc("Initial pen opacity.").addSlider((component) => component.setLimits(0, 1, 0.02).setValue(settings.defaultPenOpacity).onChange(async (value) => {
      this.plugin.settings.defaultPenOpacity = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default watercolor color").setDesc("Initial color for watercolor strokes.").addColorPicker((component) => component.setValue(settings.defaultWatercolorColor).onChange(async (value) => {
      this.plugin.settings.defaultWatercolorColor = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default watercolor width").setDesc("Initial watercolor width.").addSlider((component) => component.setLimits(MIN_BRUSH_WIDTH, MAX_BRUSH_WIDTH, 0.5).setValue(settings.defaultWatercolorWidth).onChange(async (value) => {
      this.plugin.settings.defaultWatercolorWidth = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Default watercolor opacity").setDesc("Initial watercolor opacity.").addSlider((component) => component.setLimits(0, 1, 0.02).setValue(settings.defaultWatercolorOpacity).onChange(async (value) => {
      this.plugin.settings.defaultWatercolorOpacity = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Toolbar top offset").setDesc("Extra pixels below the view header.").addSlider((component) => component.setLimits(0, 48, 1).setValue(settings.toolbarTopOffset).onChange(async (value) => {
      this.plugin.settings.toolbarTopOffset = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Debug log").setDesc("Write text-save diagnostics to the plugin folder only while troubleshooting.").addToggle((component) => component.setValue(settings.enableDebugLog).onChange(async (value) => {
...
function cleanupAllDrawingHeaderButtons() {
  activeDocument.querySelectorAll(".notedraw-header-button").forEach((button) => button.remove());
}
...
function normalizeStroke(stroke) {
  ...
  return {
    kind: stroke?.kind === TOOL_TEXT ? TOOL_TEXT : void 0,
